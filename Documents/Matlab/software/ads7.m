...
for col = 2:numCols
    ...
    % 4. 判断哪个最值离起始点更近，并决定搜索方向
    totalPoints = length(y5);  % 总数据点数
    maxRatio = maxIndex / totalPoints;  % 最大值占总数据点的比例
    minRatio = minIndex / totalPoints;  % 最小值占总数据点的比例
    
    if maxRatio < minRatio
        % 如果最大值离起始点更近，且最大值占比小于10%
        if maxRatio < 0.2  % 判断最大值是否接近起始点
            [minValue, minIndex] = min(y5(maxIndex:end));  % 向后寻找最小值
            minIndex = minIndex + maxIndex - 1;  % 修正索引
        else
            [minValue, minIndex] = min(y5(1:maxIndex));  % 向前寻找最小值
        end
    else
        % 如果最小值离起始点更近，且最小值占比小于10%
        if minRatio < 0.2  % 判断最小值是否接近起始点
            [maxValue, maxIndex] = max(y5(minIndex:end));  % 向后寻找最大值
            maxIndex = maxIndex + minIndex - 1;  % 修正索引
        else
            [maxValue, maxIndex] = max(y5(1:minIndex));  % 向前寻找最大值
        end
    end
    
    % 5. 找到原始数据对应的最大值和最小值
    maxOriginalValue = z1(maxIndex);  % 对应原始数据的最大值
    minOriginalValue = z1(minIndex);  % 对应原始数据的最小值
    ...
    % 绘制五次拟合曲线
    plot(1:length(smoothedData), y5, 'r--', 'LineWidth', 1.5, 'DisplayName', '5th Order Polynomial Fit');
    ...
    xlabel('Index');
    ylabel('Value');
    title('5th Order Polynomial Fit, Maximum and Minimum Values');
