...
%%%%%
%%%该波形文件用于生成波形两端没有衰减的波形，因此symbol_function 只能选择1   %%%%
%%%%%
...
while conditions==0
    ...
    if symbol_function==1
        if symbol_waveform==1
            ...
            Amp1=t1/length(t1);
            sig1(t1)=cos(2*pi*(fa)*t1/1000).*Amp;
            Amp3=t1(length(t1):-1:1)/length(t1);
            sig1(t3)=cos(2*pi*(fa+bandwidth)*t3/1000).*Amp;
            ...
            sig1(t2)=cos(2*pi*Phi/1000).*Amp;
            Amp1=t1/length(t1);
            sig1(t1)=cos(2*pi*(fb)*t1/1000).*Amp;
            Amp3=t1(length(t1):-1:1)/length(t1);
            sig1(t3)=cos(2*pi*(fb-bandwidth)*t3/1000).*Amp; 
            Freq=linspace(-Fs/2,Fs/2,length(t2));
            p2=fftshift(abs(fft(sig1(t2))));
