...
%%%理想值
fid1 = fopen('stage_calibration/20220106_first_row/3/ReferenceMap.csv');
...
for i=1:data_cols1
    ...
    for j=1:data_rows1
        ...
        x_array1(j,i)= x_temp1/10000.0;
        y_array1(j,i)= y_temp1/10000.0;
    end
end

%%实际值
fid2= fopen('stage_calibration/20220106_first_row/3/ActualMap.csv');
...
for i=1:data_cols2
    ...
    for j=1:data_rows2
        ...
        x_array2(j,i)= x_temp2/10000.0;
        y_array2(j,i)= y_temp2/10000.0;
    end
end
...
%%%验证数据
figure;
plot(new_y_array1(1,:),'ro');
hold on;
plot(new_y_array2(1,:),'bo');
plot(new_y_array2_correction(1,:),'go');

fid11 = fopen('stage_calibration/20220106_first_row/3/ReferenceMap_VER.csv');
...
for i=1:data_cols11
    ...
    for j=1:data_rows11
        ...
        x_temp11=str2double(temp112(1:index11-1));
        y_temp11=str2double(temp112(index11+1:end));
        
        x_array11(j,i)= x_temp11/10000.0;
        y_array11(j,i)= y_temp11/10000.0;
    end
end

%%实际值
fid22= fopen('stage_calibration/20220106_first_row/3/ActualMap_VER.csv');
...
for i=1:data_cols22
    ...
    for j=1:data_rows2
        ...
        x_array22(j,i)= x_temp22/10000.0;
        y_array22(j,i)= y_temp22/10000.0;
    end
end

fid33= fopen('stage_calibration/20220106_first_row/3/PlanMap.csv');
data_rows33=data_rows;
data_cols33=data_cols;
data33= textscan(fid33, repmat('%s',[1,data_cols33]),data_rows33,'delimiter', ',');
fclose(fid33);

x_array33=zeros(data_rows33,data_cols33);
y_array33=zeros(data_rows33,data_cols33);
for i=1:data_cols33
     temp331=data33{i};
    for j=1:data_rows33
        temp332=temp331{j};
        index33=find(temp332=='|');
        
        x_temp33=str2double(temp332(1:index33-1));
        y_temp33=str2double(temp332(index33+1:end));
        
        x_array33(j,i)= x_temp33/10000.0;
        y_array33(j,i)= y_temp33/10000.0;
    end
end

figure;
hold on;
plot( x_array1,y_array1,'r','Marker','o');
plot(x_array2,y_array2,'b','Marker','o');
plot(x_array2,new_y_array2_correction,'m','Marker','o');
plot( x_array11,y_array11,'k--','Marker','*');
plot(x_array22,y_array22,'g--','Marker','*');
%  plot( x_array33,y_array33,'r');
legend('采样点实际值','采样点理想值','采样点补偿值','扫描补偿实际值');
axis([ -240 -80 102 103]);
