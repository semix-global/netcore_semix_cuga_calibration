...
for i = 1:lengthx
    ave = mean(corrected_error_x(i,:));%mean 求解平均值
    u = std(corrected_error_x(i,:));%求解标准差
    a=[];
    b=[];
    for j = 1:lengthy
        if(abs(corrected_error_x(i,j)-ave)>3*u)%不符合 3σ准则 ， 剔除这个元素
            a(end+1)=j;
        else
            b(end+1)=corrected_error_x(i,j);
        end
    end
    if length(a)==0
        continue;
...
for i = 1:16
    ave = mean(corrected_error_y(i,:));%mean 求解平均值
    u = std(corrected_error_y(i,:));%求解标准差
    a=[];
    b=[];
    for j = 1:lengthy
        if(abs(corrected_error_y(i,j)-ave)>3*u)%不符合 3σ准则 ， 剔除这个元素
            a(end+1)=j;
            disp(a);
        else
            b(end+1)=corrected_error_y(i,j);
        end
    end
    if length(a)==0
        continue;
...
p = polyfit(1:16,corrected_error_y(17,:),6);
xi=1:1:18;
yi=polyval(p,xi);
figure(3);
plot(xi,yi,1:16,corrected_error_y(17,:),'r*');
