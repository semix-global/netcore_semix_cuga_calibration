bandwidth=50;%带宽Mhz 输入传参
for bandwidth =50:2:150
    % 280输入传参
fa=280-bandwidth/2;fb=280+bandwidth/2; %a低频，b高频
fa1=fa;fb1=fb;

L=6;%L 音包长度,mm 输入传参
T=round(L/5.742*1000);%平坦时间ns % prescan 输入传参，Chirp 用L算
symbol_waveform=0;% 0代表递减, 1代表递增 % 输入传参 
...
if symbol_AOD==0
    ...
    name='chirp';
    result=sprintf('%s_%dmm_%dM_%dM_%dns_%dAMP_%d.txt', name,L,fa,fb,T,Amp,N);
end
conditions=0;
j=0;
