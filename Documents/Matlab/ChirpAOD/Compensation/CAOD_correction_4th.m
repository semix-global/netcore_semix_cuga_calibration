...
v = 5.742; Delta = -10; %
delay = 0;
fa = 345 + Delta; fb = 308 + Delta; %a高频，b低频；开放功能
T_chirp = L / v * 1000;
ramp = (fa - fb) / T_chirp; %MHz,ns   1/5.742=174;1.5/5.742=261
% Fs=1000;%Fs,M
Fs = 1064; %Fs,M；采样率

N = round(1000 * L / v * Fs / 1000 + 2 * delay);
t = (1:N)'; %ns
t1 = (1:delay)';
t2 = (delay + 1:N - delay)';
t3 = (N - delay + 1:N)';
...
for E = values_e
    Phi1 = ramp / 2 * C * (t2 - (length(t2) / 2)) .^ 2 + ramp / 2 * D * (t2 - (length(t2) / 2)) .^ 3 + ramp / 2 * E * (t2 - (length(t2) / 2)) .^ 4; %中间

    %     Phi1=ramp/2*C*(t2-length(t2)).^2+ramp/2*D*(t2-length(t2)).^3; %起始
    %     Phi1=ramp/2*C*(t2-t2(1)).^2+ramp/2*D*(t2-t2(1)).^3; %末尾

    Phi2 = fa - ramp / 2 * (t2 - t2(1));

    %     Phi3 = ramp/2*D*(t2-(length(t2)/2)).^3;
    ...
    filename = sprintf('B3_CAOD_3.2mm_355_205_557ns_0delay_C%.6f_D%.9f_E%.12f.txt', C, D, E);
    % 打开文件用于写入
