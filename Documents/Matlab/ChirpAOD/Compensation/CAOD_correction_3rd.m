...
v = 5.742; Delta = 0; %
delay = 0;
fa = 345 + Delta; fb = 245 + Delta; %a高频，b低频；开放功能
T_chirp = L / v * 1000;
...
for D = values_d
    Phi1 = ramp / 2 * C * (t2 - ((length(t2)) / 2)) .^ 2 + ramp / 2 * D * (t2 - ((length(t2)) / 2)) .^ 3; %中间
    ...
    %     Phi3 = ramp/2*D*(t2-((length(t2)+31)/2)).^3;
    Phi = Phi2 - Phi1;
    ...
    filename = sprintf('B3_CAOD_3.2mm_355_205_557ns_0delay_0.001C_%.8f_D.txt', D);
    % 打开文件用于写入
