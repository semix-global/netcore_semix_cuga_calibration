function Output = ChirpAOD(bandwidth, L, symbol_AOD, directory)
    fa = 280 - bandwidth / 2; fb = 280 + bandwidth / 2; %a低频，b高频
    fa1 = fa; fb1 = fb;

    % T = round(L / 5.742 * 1000); %平坦时间ns
    T = 4300
    symbol_waveform = 0; % 0代表递减, 1代表递增
    symbol_function = 1; % 0代表选择波形的起始频率从0处就计算 。1代表波形加入了delay，波形的起始频率从delay结束后开始计算
    ...
    if symbol_AOD == 0
        name = 'prescan';
        result = sprintf('%s\\%s_%dM_%dns_%dAMP_%d.txt', directory, name, bandwidth, T, Amp, N);
    else
        name = 'chirp';
        result = sprintf('%s\\%s_%dmm_%dM_%dM_%dns_%dAMP_%d.txt', directory, name, L, fa, fb, T, Amp, N);
    end
    ...
    while conditions == 0
        ...
        if symbol_function == 1
            ...
            if symbol_waveform == 1
                Phi = fa + ramp / 2 * (t2 - t2(1)) + c * ramp / 2 * (t2 - ((t2(1) + t2(end)) / 2)) .^ 2;
                sig1(t2) = cos(2 * pi * Phi .* t2 / Fs) .* Amp;
                Amp1 = t1 / length(t1);
                sig1(t1) = cos(2 * pi * (fa) * t1 / Fs) .* Amp1;
                Amp3 = t1(length(t1):-1:1) / length(t1);
                sig1(t3) = cos(2 * pi * (fb) * t3 / Fs) .* Amp3;
                ...
                if abs(Freq(x1) - fa1) < (Fs / length(t2))
                    ...
                    if abs(Freq(x2) - fb1) < (Fs / length(t2))
                        conditions = 1;
                    elseif Freq(x1) < fb1
                        bandwidth1 = bandwidth1 - Fs / (2 * length(t));
                    else
                        bandwidth1 = bandwidth1 + Fs / (2 * length(t));
                    end
                ...
                Phi = fb - ramp / 2 * (t2 - t2(1)) - c * ramp / 2 * (t2 - ((t2(1) + t2(end)) / 2)) .^ 2;
                sig1(t2) = cos(2 * pi * Phi .* t2 / Fs) .* Amp;
                Amp1 = t1 / length(t1);
                sig1(t1) = cos(2 * pi * (fb) * t1 / Fs) .* Amp1;
                Amp3 = t1(length(t1):-1:1) / length(t1);
                sig1(t3) = cos(2 * pi * (fa) * t3 / Fs) .* Amp3;
                Freq = linspace(-Fs / 2, Fs / 2, length(t2));
                p2 = fftshift(abs(fft(sig1(t2))));
