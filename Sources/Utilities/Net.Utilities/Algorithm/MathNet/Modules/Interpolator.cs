...
public static class Interpolator
{
    ...
    {
        if (t < left.X || right.X < t || left.X > right.X) ThrowHelper.ThrowArgumentOutOfRangeException<double>("The point t is outside the interval.");

        var interpolation = global::MathNet.Numerics.Interpolate.Linear([left.X, right.X], [left.Y, right.Y]);
    ...
    public static double Bilinear(Point3D leftDown, Point3D rightDown, Point3D leftUp, Point3D rightUp, Point t)
    {
        ...
        void Verify()
        {
            ...
            // 检查对边是否相等
            var isEqual = Math.Abs(v1.L2Norm() - v3.L2Norm()) < ConstantHelper.Tolerance && Math.Abs(v2.L2Norm() - v4.L2Norm()) < ConstantHelper.Tolerance;

            // 检查两两是否为直角（点积为零）
            var isZero = Math.Abs(v1.DotProduct(v2)) < ConstantHelper.Tolerance
                         && Math.Abs(v2.DotProduct(v3)) < ConstantHelper.Tolerance
                         && Math.Abs(v3.DotProduct(v4)) < ConstantHelper.Tolerance;

            if (isEqual == false || isZero == false) ThrowHelper.ThrowArgumentOutOfRangeException<double>("The is not a rectangle.");

            // 必须内部或者边上
            if (t.X < leftDown.X || rightDown.X < t.X || t.Y < leftDown.Y || leftUp.Y < t.Y || leftDown.X > rightDown.X || leftDown.Y > leftUp.Y)
                ThrowHelper.ThrowArgumentOutOfRangeException<double>("The point t is outside the rectangle.");
        }
    }
