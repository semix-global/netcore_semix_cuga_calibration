...
{
    public static (Vector<double> X, Vector<double> Y) SplineInterpolation(Vector<double> x, Vector<double> y, int densityFactor, int order = 3)
    {
        ...
        for (var i = 0; i < x.Count - 1; i++)
        {
            ...
            for (var j = 1; j < densityFactor; j++)
            {
                var t = (double)j / densityFactor;
                var xInterp = x1 + t * (x2 - x1);
