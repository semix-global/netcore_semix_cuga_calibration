    ...
    private void LoadCalibrationStatus()
    {
        _ = Task.Run(() =>
        {
            try
            {
                ...
                calibrationItem = _applicationCookieService.FindCalibrationItem<ChuckBrightFieldStageMapCalibrationViewModel>();
                ...
                calibrationItem = _applicationCookieService.FindCalibrationItem<LaserAttenuatorCalibrationViewModel>();
                if (calibrationItem is not null) calibrationItem.IsCalibrated = _cacheProvider.GetOrDefaultArray<LaserAttenuatorObjDto>().IsOk(out _);
                ...
                calibrationItem = _applicationCookieService.FindCalibrationItem<LaserAodDelayCalibrationViewModel>();
                if (calibrationItem is not null) calibrationItem.IsCalibrated = _cacheProvider.GetOrDefaultArray<LaserAodDelayItemDto>().IsOk(out _);
                ...
                calibrationItem = _applicationCookieService.FindCalibrationItem<LaserOpticalPowerMeterCalibrationViewModel>();
                ...
                if (calibrationItem is not null) calibrationItem.IsCalibrated = _cacheProvider.GetOrDefaultArray<LaserPrescanChirpAodAlignmentDto>().IsOk(out _);
