...
public sealed partial class PupilSideChannelFlexibleApertureViewModel
{
    private bool LoadDepends()
    {
        ...
        var pupilCameraAlignmentViewModel = HostApplication.GetRequiredService<PupilCameraAlignmentViewModel>();
        if (pupilCameraAlignmentViewModel.Entry.Status.IsOk == false)
        {
            DialogWindowProvider.ShowDialog($"The {pupilCameraAlignmentViewModel.Name} precondition is Not Ok", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }
