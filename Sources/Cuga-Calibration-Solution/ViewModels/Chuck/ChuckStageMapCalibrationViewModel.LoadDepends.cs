...
public sealed partial class ChuckStageMapCalibrationViewModel
{
    private bool LoadDepends()
    {
        ...
        if (CalibrationStatusService.GetCalibrationDtoItemsIsOKStatus<AODUniformityDTO>(out _, out errorMessage) == false)
        {
            DialogWindowProvider.ShowDialog($"precondition is Failure,Error:{errorMessage}", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }
        ...
        if (CalibrationStatusService.GetCalibrationDtoItemsIsOKStatus<CIBLightMatchingDTO>(out _, out errorMessage) == false)
        {
            DialogWindowProvider.ShowDialog($"precondition is Failure,Error:{errorMessage}", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }
        ...
        if (CalibrationStatusService.GetCalibrationDtoItemsIsOKStatus<ChuckAlignmentDegreeOffsetItemDto>(out _, out errorMessage) == false)
        {
            DialogWindowProvider.ShowDialog($"precondition is Failure,Error:{errorMessage}", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }
