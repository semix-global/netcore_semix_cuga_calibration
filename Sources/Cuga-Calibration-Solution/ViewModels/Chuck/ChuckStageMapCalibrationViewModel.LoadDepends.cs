...
public sealed partial class ChuckStageMapCalibrationViewModel
{
    private bool LoadDepends()
    {
        ...
        var cIBIlluminationProfileViewModel = HostApplication.GetRequiredService<CIBIlluminationProfileViewModel>();
        if (cIBIlluminationProfileViewModel.Entry.Status.IsOk == false)
        {
            DialogWindowProvider.ShowDialog($"The {cIBIlluminationProfileViewModel.Name} precondition is Not Ok", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }

        var cIBLightMatchingViewModel = HostApplication.GetRequiredService<CIBLightMatchingViewModel>();
        if (cIBLightMatchingViewModel.Entry.Status.IsOk == false)
        {
            DialogWindowProvider.ShowDialog($"The {cIBLightMatchingViewModel.Name} precondition is Not Ok", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }

        var cIBYPixelSizeViewModel = HostApplication.GetRequiredService<CIBYPixelSizeViewModel>();
        if (cIBYPixelSizeViewModel.Entry.Status.IsOk == false)
        {
            DialogWindowProvider.ShowDialog($"The {cIBYPixelSizeViewModel.Name} precondition is Not Ok", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }

        var cIBXPixelSizeViewModel = HostApplication.GetRequiredService<CIBXPixelSizeViewModel>();
        if (cIBXPixelSizeViewModel.Entry.Status.IsOk == false)
        {
            DialogWindowProvider.ShowDialog($"The {cIBXPixelSizeViewModel.Name} precondition is Not Ok", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }

        var cIBLineCentricityViewModel = HostApplication.GetRequiredService<CIBLineCentricityViewModel>();
        if (cIBLineCentricityViewModel.Entry.Status.IsOk == false)
        {
            DialogWindowProvider.ShowDialog($"The {cIBLineCentricityViewModel.Name} precondition is Not Ok", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }

        var cIBLineOrientationOffsetViewModel = HostApplication.GetRequiredService<CIBLineOrientationOffsetViewModel>();
        if (cIBLineOrientationOffsetViewModel.Entry.Status.IsOk == false)
        {
            DialogWindowProvider.ShowDialog($"The {cIBLineOrientationOffsetViewModel.Name} precondition is Not Ok", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }

        var chuckAlignmentDegreeOffsetViewModel = HostApplication.GetRequiredService<ChuckAlignmentDegreeOffsetCalibrationViewModel>();
        if (chuckAlignmentDegreeOffsetViewModel.Entry.Status.IsOk == false)
        {
            DialogWindowProvider.ShowDialog($"The {chuckAlignmentDegreeOffsetViewModel.Name} precondition is Not Ok", DialogButtonsEnum.OK, DialogIconEnum.Warning);
            return false;
        }
