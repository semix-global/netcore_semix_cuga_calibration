    ...
    IOptions<ApplicationSetting> options) : CalibrationViewModelBase
{
    ...
    public override string CalibrateDirectoryName => $"{EnumHelper.ToDescriptionString(Cache.OpticsMagTypeEnum)}-{Cache.Coefficient:f3}";

    public override string CalibrateFileName => $"{EnumHelper.ToDescriptionString(Cache.OpticsMagTypeEnum)}-{Cache.Coefficient:f3}";
    ...
    [ObservableProperty]
    private ObservableCollection<OpticsMagTypeEnumAndCoefficientCalibrationStatus> _calibrationStatusList =
    [
        ..EnumHelper.Enums<OpticsMagTypeEnum>().Select(t => new OpticsMagTypeEnumAndCoefficientCalibrationStatus { OpticsMagTypeEnum = t, CoefficientList = [.. CoefficientCalibrationStatus.CreateList()] })
    ];

    [ObservableProperty]
    private ObservableCollection<CoefficientCalibrationStatus> _calibrationStatusListItem = [.. CoefficientCalibrationStatus.CreateList()];
    ...
    [ObservableProperty]
    private List<LaserIlluminationProfileCalibrationPmtIdItem> _pmtIdItemList = [];
    ...
    protected override async Task<bool> LoadedingAsync(CancellationToken cancellationToken)
    {
        ...
        {
            CalibrationStatusList.Single(t => t.OpticsMagTypeEnum == calibrationStatus.OpticsMagTypeEnum)
                .CoefficientList.Single(t => t.Coefficient - calibrationStatus.Coefficient == 0)
        ...
        switch (CalibrationStepIndex)
        {
            ...
            case 6:
                ...
                CalibrationStatusList.Single(t => t.OpticsMagTypeEnum == Cache.OpticsMagTypeEnum)
                    .CoefficientList.Single(t => t.Coefficient - Cache.Coefficient == 0)
                    .IsCalibrated = true;
    ...
    private void ChangePrescanFile()
    {
        ...
        Cache.CurrentDarkFieldImageListToPrescanListCacheItem.PrescanFilePath = filePath;
    }

    [RelayCommand]
    private void ReviewImage(List<string> filePaths)
    {
        if (filePaths.Count == 0 && filePaths.Count != PmtIdItemList.Count) return;
        var filePathList = new List<(string, string)>();
        for (var i = 0; i < PmtIdItemList.Count; i++)
        {
            var filePath = (filePaths[i], PmtIdItemList[i].PmtId.ToString());
    ...
    [RelayCommand]
    private void ReviewPlot(List<List<double>> list)
    {
        if (list.Count == 0 && list.Count != PmtIdItemList.Count + 1) return;
    ...
    private async Task SavePrescanAodWaveAsync(LaserIlluminationProfileItemDto? laserIlluminationProfileItemDto)
    {
        try
        {
            await Task.Run(() =>
            {
                ...
                var prescanDto = LaserViewModel.ReadPrescanByFile(laserIlluminationProfileItemDto.PrescanFilePath, Cache.Coefficient);

                var hexStrings = prescanDto.PrescanList
                    .Select((value, i) => (short)(value * laserIlluminationProfileItemDto.PrescanRateList[i]))
                    .Select(t => t.ToString("X4")) // 转换为16进制字符串, 至少4位不足右边补0
                    .Select(t => t[^4..])
                    .ToArray();

                File.WriteAllText($"{FileHelper.GetFileFullName(saveFilePath)}{Path.GetFileName(laserIlluminationProfileItemDto.PrescanFilePath)}", string.Join(Environment.NewLine, hexStrings));
    ...
    private Task Step2CalibrateActionAsync(CancellationToken cancellationToken)
    {
        ...
        {
            PmtIdItemList.Clear();
            Cache.CurrentCalibrationCacheItem.LaserIlluminationProfileCalibrationPmtList.Clear();
            var pmt8 = new LaserIlluminationProfileCalibrationPmtIdItem
            {
                PmtId = 8,
                ChannelId = Cache.ChannelId,
                PmtIdPosition = Cache.FindPosition,
            };
            Cache.CurrentCalibrationCacheItem.LaserIlluminationProfileCalibrationPmtList.Add(pmt8);
            for (var i = 7; i >= 1; i--)
            {
                var pmt = new LaserIlluminationProfileCalibrationPmtIdItem
                {
                    PmtId = i,
                    ChannelId = Cache.ChannelId,
            ...
            {
                PmtIdItemList = [.. Cache.CurrentCalibrationCacheItem.LaserIlluminationProfileCalibrationPmtList.Where(t => pmtConfig[t.PmtId - 1].Enabled)];
            }
            ...
            foreach (var pmtItem in PmtIdItemList)
            {
                var (isSuccess, gain) = await AutoGainSettingDarkFieldGainViewModel.AutoPmtGainAsync(Cache.Coefficient, Cache.FindPosition, CalChipSiteModelEnum.HazeModel, HtmlLogUniqueId, cancellationToken, false, pmtItem.PmtId, pmtItem.ChannelId).ConfigureAwait(false);
                if ((isSuccess) == false)
                {
                ...
                Logger.LogHtmlInformation($"PmtId: {pmtItem.PmtId}", HtmlHeaderLevelEnum.Header4, new HtmlQuote(new
                {
                    pmtItem.PmtId,
                    pmtItem.ChannelId,
                    Findposition = pmtItem.PmtIdPosition,
                    Gain = gain
                }), HtmlLogUniqueId.LoggingHtml());
            }
    ...
    private Task Step3CalibrateActionAsync(CancellationToken cancellationToken)
    {
        return InvokeCalibrateAsync(async () =>
        {
            ...
            var prescanFilePath = item.PrescanFilePath;
            ...
            var prescanDto = LaserViewModel.ReadPrescanByFile(prescanFilePath, Cache.Coefficient);
            ...
            var (isSuccess, channel1DarkFieldImageDto, channel2DarkFieldImageDto, channel3DarkFieldImageDto) = GetDarkFieldLineScanImage(temp, Cache.PmtId, Cache.FindPosition);
            ...
            temp.PrescanByteList = prescanByteList;
            (isSuccess, channel1DarkFieldImageDto, channel2DarkFieldImageDto, channel3DarkFieldImageDto) = GetDarkFieldLineScanImage(temp, Cache.PmtId, Cache.FindPosition);
            ...
            if (item.PrescanStartIndex < 0
                || prescanDto.PrescanList.Count <= item.PrescanStartIndex
                || item.PrescanEndIndex < 0
                || prescanDto.PrescanList.Count <= item.PrescanEndIndex)
            {
            ...
            (List<byte> PrescanByteList, List<double> WindowPrescanList) SetPrescanByteListByWindow(int startIndex, int windowToMinAmountTemp)
            {
                ...
                var coefficient = Cache.Coefficient;
    ...
    private Task Step4CalibrateActionAsync(CancellationToken cancellationToken)
    {
        return InvokeCalibrateAsync(() =>
        {
            ...
            {
                var compArray = BitConverter.GetBytes((short)(prescanDto.PrescanList[i] * coefficient));
                resultPrescanByteList.Add(compArray[1]);
                resultPrescanByteList.Add(compArray[0]);
                resultPrescanWindowList.Add(coefficient);
            }
            ...
            for (var i = waveFormVEndIndex; i < prescanDto.PrescanList.Count; i++) //结束部分按照原来系数计算
            {
                var compArray = BitConverter.GetBytes((short)(prescanDto.PrescanList[i] * coefficient));
                resultPrescanByteList.Add(compArray[1]);
                resultPrescanByteList.Add(compArray[0]);
    ...
    private Task Step5CalibrateActionAsync(CancellationToken cancellationToken)
    {
        return InvokeCalibrateAsync(async () =>
        {
            ...
            var prescanFilePath = darkFieldImageListToPrescanListCacheItem.PrescanFilePath;
            ...
            foreach (var c in Enumerable.Range(0, 11).Select(t => 0.5 * Cache.Coefficient + t * 0.05 * Cache.Coefficient))
            {
                ...
                var prescanDtoTemp = LaserViewModel.SetUploadPrescanListByRate(prescanDto, [.. Vector<double>.Build.Dense(prescanDto.PrescanList.Count, c)]);

                var (isSuccess, channel1DarkFieldImageDto, channel2DarkFieldImageDto, channel3DarkFieldImageDto) = GetDarkFieldLineScanImage(prescanDtoTemp, 8, Cache.FindPosition);
                using var _1 = channel1DarkFieldImageDto;
                using var _2 = channel2DarkFieldImageDto;
                using var _3 = channel3DarkFieldImageDto;
                ...
                PlotList = [.. PlotList, new WpfPlotModel($"{c:f3}", servingToDarkFieldImageListAverages.ToPoints(), (0.5 * Cache.Coefficient, Cache.Coefficient, c))];
            }
    ...
    private Task Step6CalibrateActionAsync(CancellationToken cancellationToken)
    {
        return InvokeCalibrateAsync(async () =>
        {
            ...
            var cacheCoefficientLimitMin = IlluminationCoefficientLimitMin = Math.Max(0, Cache.Coefficient - Cache.Coefficient * coefficientLimit);
            var cacheCoefficientLimitMax = IlluminationCoefficientLimitMax = Math.Min(1d, Cache.Coefficient + Cache.Coefficient * coefficientLimit);
            ...
            var prescanDtoTemp = LaserViewModel.SetUploadPrescanListByRate(prescanDto, illuminationIntensityConsistentDto.PrescanRateList);
            ...
            foreach (var index in Enumerable.Range(1, repeatCount))
            {
                ...
                if (TryCheckIlluminationIsOk(detectImageDirectory, prescanDto, tempIlluminationProfileDto, out var isOk) == false) return false;

                if (isOk) return await GetResultAsync().ConfigureAwait(false);

                var darkFieldImageList = tempIlluminationProfileDto.ChannelDarkFieldImageList;
                ...
                targetServing ??= Cache.Coefficient < targetCoefficient
                ...
                targetValue ??= IlluminationTargetValue = Cache.Coefficient < targetCoefficient
                    ? targetAverage
                ...
                if (tempIlluminationProfileDto.ChannelDarkFieldPmtList.Count == 0 && tempIlluminationProfileDto.ChannelDarkFieldPmtList.Count != PmtIdItemList.Count + 1) return false;
                ...
                Logger.LogHtmlInformation($"{tempIlluminationProfileDto.Index}", HtmlHeaderLevelEnum.Header4, new HtmlBullet(new
                {
                    ...
                    DarkFieldImageCh3List = new HtmlPlot2DLinesChart(GetChannelDarkFieldImageList(tempIlluminationProfileDto.ChannelDarkFieldPmtList).ToArray(), "DarkFieldImageCh3List"),
            ...
            async Task<bool> GetResultAsync()
            {
                ...
                var hexStrings = prescanDto.PrescanList
                    .Select((value, i) => (short)(value * SelectCalibrateItemDto.PrescanRateList[i]))
                    .Select(t => t.ToString("X4")) // 转换为16进制字符串, 至少4位不足右边补0
                    .Select(t => t[^4..])
                    .ToArray();

                SelectCalibrateItemDto.ResultPrescanFilePath = Path.Combine(PrescanFileDirectory,
                    $"{Path.GetFileNameWithoutExtension(SelectCalibrateItemDto.PrescanFilePath)}_{EnumHelper.ToDescriptionString(SelectCalibrateItemDto.OpticsMagTypeEnum)}_{SelectCalibrateItemDto.Coefficient:f3}.txt");
                DirectoryHelper.CreateFileDirectoryIfNotExists(SelectCalibrateItemDto.ResultPrescanFilePath);
                File.WriteAllText(SelectCalibrateItemDto.ResultPrescanFilePath, string.Join(Environment.NewLine, hexStrings));

                prescanDtoTemp = LaserViewModel.SetUploadPrescanListByRate(prescanDto, SelectCalibrateItemDto.PrescanRateList);
                ...
                Logger.LogHtmlHeaderIsOk(HtmlHeaderLevelEnum.Header3, new HtmlBullet(new
                {
                    SelectCalibrateItemDto.ResultPrescanFilePath,
                    ...
                    DarkFieldImageList = new HtmlPlot2DLinesChart(GetChannelDarkFieldImageList(SelectCalibrateItemDto.ChannelDarkFieldPmtList).ToArray(), "DarkFieldImageList"),
                    HtmlTab = new HtmlTab(new
                    {
    ...
    private async Task VerifyActionAsync(CancellationToken cancellationToken)
    {
        ...
        await InvokeVerifyAsync(() =>
        {
            ...
            Cache.Coefficient = SelectReviewItemDto.Coefficient;
            ...
            var prescanDto = LaserViewModel.ReadPrescanByFile(prescanFilePath, SelectReviewItemDto.Coefficient);
    ...
    private bool TryCheckIlluminationIsOk(string detectImageDirectory, DarkFieldPrescanDto prescanDto, in LaserIlluminationProfileItemDto laserIlluminationProfileItemDto, out bool isOk, bool isCalibrate = true)
    {
        var darkFieldImageListToPrescanListCacheItem = Cache.CurrentDarkFieldImageListToPrescanListCacheItem;
        var judgeDarkFieldImageListRateSkipCout = Cache.CurrentCalibrationCacheItem.JudgeDarkFieldImageListRateSkipCout;

        isOk = false;
        var prescanDtoTemp = LaserViewModel.SetUploadPrescanListByRate(prescanDto, laserIlluminationProfileItemDto.PrescanRateList);

        var channelList = new List<List<double>>();
        var channelDarkFieldImageList = new List<string>();
        foreach (var pmtItem in PmtIdItemList)
        {
            LaserViewModel.SetGain(pmtItem.Gain);

            Thread.Sleep(1000);
            var (isSuccess, channel1DarkFieldImageDto, channel2DarkFieldImageDto, channel3DarkFieldImageDto) = GetDarkFieldLineScanImage(prescanDtoTemp, pmtItem.PmtId, pmtItem.PmtIdPosition);
            using var _1 = channel1DarkFieldImageDto;
            using var _2 = channel2DarkFieldImageDto;
            using var _3 = channel3DarkFieldImageDto;
            if (isSuccess == false) return false;
            ...
            laserIlluminationProfileItemDto.Channel2ImageFilePath = $"{detectImageDirectory}\\({HtmlLogUniqueId}_{middleFileDateTimeFormat}_PmtId_{pmtItem.PmtId}_Channel2_{laserIlluminationProfileItemDto.Index}).jpg";
            HalconHelper.Save(channel2DarkFieldImageDto.Image, laserIlluminationProfileItemDto.Channel2ImageFilePath);
            laserIlluminationProfileItemDto.Channel3ImageFilePath = $"{detectImageDirectory}\\({HtmlLogUniqueId}_{middleFileDateTimeFormat}_PmtId_{pmtItem.PmtId}_Channel3_{laserIlluminationProfileItemDto.Index}).jpg";
            ...
            Logger.LogHtmlInformation($"PmtId: {pmtItem.PmtId}", HtmlHeaderLevelEnum.Header4, new HtmlQuote(new
            {
                ...
                pmtItem.PmtId,
                pmtItem.ChannelId,
                pmtItem.PmtIdPosition,
                pmtItem.Gain,
                DarkFieldImageList = new HtmlPlot2DLinesChart([
                    ("ch1", channel1DarkFieldImageDto.ProjectionYs.ToPoints()), ("ch2", channel2DarkFieldImageDto.ProjectionYs.ToPoints()),
            ...
            if (Cache.ChannelId == 1)
            {
                channelDarkFieldImageList.Add(laserIlluminationProfileItemDto.Channel1ImageFilePath);
                channelList.Add([.. channel1DarkFieldImageDto.ProjectionYs]);
            }

            if (Cache.ChannelId == 2)
            {
                channelDarkFieldImageList.Add(laserIlluminationProfileItemDto.Channel2ImageFilePath);
                channelList.Add([.. channel2DarkFieldImageDto.ProjectionYs]);
            }

            if (Cache.ChannelId == 3)
            {
                channelDarkFieldImageList.Add(laserIlluminationProfileItemDto.Channel3ImageFilePath);
                channelList.Add([.. channel3DarkFieldImageDto.ProjectionYs]);
            }
        }

        laserIlluminationProfileItemDto.ChannelImageFilePathList = channelDarkFieldImageList;
        laserIlluminationProfileItemDto.ChannelDarkFieldPmtList = channelList;

        var averageMaxChannelList = new List<double>();
        for (var i = 0; i < channelList[0].Count; i++)
        {
            double sumNum = 0;
            foreach (var channelPlot in channelList)
            {
                sumNum += channelPlot[i];
            }

            averageMaxChannelList.Add(sumNum / channelList.Count);
        }

        laserIlluminationProfileItemDto.ChannelDarkFieldPmtList.Add(averageMaxChannelList);
        laserIlluminationProfileItemDto.ChannelDarkFieldImageList = Cache.GetDarkFieldImageList(averageMaxChannelList);
        var darkFieldImageList = laserIlluminationProfileItemDto.ChannelDarkFieldImageList;
    ...
    private (bool IsSuccess,
        ...
        GetDarkFieldLineScanImage(DarkFieldPrescanDto darkFieldPrescanDto, int pmtId = 8, Point position = default)
    {
    ...
    private List<(string, Point[])> GetChannelDarkFieldImageList(List<List<double>> result)
    {
        var plotLists = new List<(string, Point[])>();
        for (var i = 0; i < PmtIdItemList.Count; i++)
        {
            var plot = (PmtIdItemList[i].PmtId.ToString(), Cache.GetDarkFieldImageList(result[i]).ToPoints());
            plotLists.Add(plot);
        }

        plotLists.Add(("Average", Cache.GetDarkFieldImageList(result[PmtIdItemList.Count]).ToPoints()));
        return plotLists;
    }
    ...
    private bool ToIlluminationIntensity(LaserIlluminationProfileItemDto itemDto, CancellationToken cancellationToken) => InvokeSave(update =>
    {
        ...
        [
            .. Calibrations
                .Where(t => (t.PmtId == itemDto.PmtId && t.OpticsMagTypeEnum == itemDto.OpticsMagTypeEnum && t.Coefficient - itemDto.Coefficient == 0) == false),
            itemDto.Clone(),
        ];
