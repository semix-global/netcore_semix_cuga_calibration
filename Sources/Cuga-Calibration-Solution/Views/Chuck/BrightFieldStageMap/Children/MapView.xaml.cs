...
public sealed partial class MapView
{
    ...
    private void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        if (DataContext is not ChuckBrightFieldStageMapCalibrationViewModel viewModel) return;
    ...
    {
        if (sender is not ChuckBrightFieldStageMapCalibrationViewModel viewModel) return;
        ...
        switch (e.PropertyName)
        {
                    ...
                    case "Step3View" when viewModel.CalibrationStepIndex is 1 or 2 or 3:
                        Dispatcher.Invoke(() =>
                        {
                            ShowVectorField(WpfPlot,
                                viewModel.ResultChuckBrightFieldStageMapDto.CalibrationStageMap.IdealStageMapItemMatrix,
                                viewModel.ResultChuckBrightFieldStageMapDto.CalibrationStageMap.ErrorMatrix,
                                viewModel.ResultChuckBrightFieldStageMapDto.CalibrationStageMap.RowNumber,
                                viewModel.ResultChuckBrightFieldStageMapDto.CalibrationStageMap.ColumnNumber,
                ...
                switch (ChildName)
                {
                    case "ReviewCalibration":
                        Dispatcher.Invoke(() =>
                        {
                            ...
                            ShowVectorField(WpfPlot,
                                viewModel.ReviewDto.CalibrationStageMap.IdealStageMapItemMatrix,
                                viewModel.ReviewDto.CalibrationStageMap.ErrorMatrix,
                                viewModel.ReviewDto.CalibrationStageMap.RowNumber,
                                viewModel.ReviewDto.CalibrationStageMap.ColumnNumber,
                                Colors.Green.WithAlpha(0.3),
                                viewModel.ChuckCenter.NewBFCenterStagePosition,
                                viewModel.Cache.WaferDiameter);
                        });
                        break;
            ...
            case nameof(viewModel.ReviewDto.VerifyStageMap):
                switch (ChildName)
                {
                    case "ReviewVerify":
                        Dispatcher.Invoke(() =>
                        {
                            ...
                            ShowVectorField(WpfPlot,
                                viewModel.ReviewDto.VerifyStageMap.IdealStageMapItemMatrix,
                                viewModel.ReviewDto.VerifyStageMap.ErrorMatrix,
                                viewModel.ReviewDto.VerifyStageMap.RowNumber,
                                viewModel.ReviewDto.VerifyStageMap.ColumnNumber,
                                Colors.Green.WithAlpha(0.3),
                                viewModel.ChuckCenter.NewBFCenterStagePosition,
                                viewModel.Cache.WaferDiameter);
                        });
        ...
        try
        {
            ...
            for (var row = 0; row < rowNumber; row++)
            {
                for (var column = 0; column < columnNumber; column++)
                {
                    ...
                    var labelBackgroundColor = idealItem.IsInWafer == false
                        ? Colors.DarkRed
    ...
    private static void ConfigureWpfPlot(WpfPlot wpfPlot, ChuckBrightFieldStageMapCalibrationViewModel viewModel)
    {
