...
def process_first_measurement(
    ...
    参数:
        residual: 第一次测量得到的二维误差场
            ``E = P_measured - P_desired``，形状为 ``(..., 2)``。最后一维
            依次为 X、Y 残差，中间维度是测量网格。
        desired_positions: 网格点的期望二维坐标，形状为 ``(..., 2)``，必须
            与 ``residual`` 完全相同。最后一维依次为 X、Y 坐标。
...
def process_stage2_residuals(
    ...
    参数:
        residuals: 累计二维残差数组，形状为 ``(M, ..., 2)``。第0维是扫描
            序号，最后一维依次为 X、Y 残差，中间维度是网格。每次扫描必须
            具有相同的网格形状，且值均为有限数。
        desired_positions: 网格点的期望二维坐标，形状为 ``(..., 2)``，必须
            与单次残差的形状完全相同。最后一维依次为 X、Y 坐标。二维非
            共线网格执行完整仿射拟合；单行等共线点集只扣除 X、Y 分量均值。
    ...
    # 数据布局约定：
    #   values.shape    == (扫描次数, 网格维度..., 2)
    #   positions.shape == (网格维度..., 2)
    # 最后一维长度 2 分别代表 X、Y；中间网格可以是一维点列，也可以是二维网格。
    ...
    # 以下统计均沿第 0 维（扫描序号）进行，网格位置及 X/Y 分量保持不变。
