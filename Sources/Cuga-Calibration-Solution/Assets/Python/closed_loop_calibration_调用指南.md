...
阶段1测量 residual + stage1_mask
        ↓
生成校准源网格 C0，并将 mask=False 点填入有限的初始值
        ↓
...
合并 C0_download 和 deltaC，得到 C_final + interpolation_mask
        ↓
使用 interpolation_mask 将 C_final 插值到最终下发网格
        ↓
...
2. 阶段 2 每次扫描的 mask 可以不同，外部程序必须保存完整的 `residuals` 和 `masks` 历史，不能只传最后一次扫描的 mask。
3. 阶段 1、阶段 2 都无效的点可以在设备表中保留 fallback 值，但会在 `interpolation_mask` 中标记为无效，不能参与后续插值。
...
| 操作 | 输入 | 输出 |
| --- | --- | --- |
| 阶段 1 处理 | `residual`、`positions`、`stage1_mask`、`fill_value` | 完整有限的源网格 `C0` |
| 初始插值 | `C0`、源网格坐标、下发网格坐标 | 实际下发的 `C0_download` |
| 阶段 2 处理 | 累计 `residuals`、`masks`、阶段 2 坐标及参数 | `need_more`、`deltaC` |
| 合并和最终插值 | 初始表、`deltaC`、质量 mask、目标坐标 | `final_download_table`、`interpolation_mask` |
...
- `mask=False` 的残差可以传 NaN，`mask=True` 的残差必须是有限值；
- 外部程序可以使用 JSON、二进制、共享内存或其他方式传输，但必须保留数组形状、数据类型、最后一维分量顺序和 NaN 信息。

返回状态建议按以下方式处理：

- 正常返回：使用返回的数组；
- `need_more=True`：继续采集阶段 2，不生成最终下发表；
- `RuntimeWarning`：记录警告并按策略继续，例如三角点插值或达到 `m_max`；
...
阶段 2 的残差、`deltaC`、合并时的初始表和阶段 1 质量 mask 必须属于同一个 `stage2_positions` 网格。最简单的情况是阶段 2 就在初始下发网格上扫描，此时 `stage2_positions = download_positions`。
...
`mask=False` 的点不参与阶段 1 仿射拟合，源网格上对应的 `C0_on_calibration_grid` 使用 `fill_value`，默认是 `0.0`。这些填充值是阶段 2 的初始估计，因此在这一次初始插值中应让它们参与计算；不要在这里传入 `source_mask=stage1_mask`。

`C0_download` 才是设备实际使用的完整表：
...
`fill_value` 必须是有限标量，并同时用于该点的 X/Y 两个修正分量。
...
while True:
    ...
    # 可选：将无效点显式写成 NaN，便于外部查看；算法本身会依据 mask 忽略它们。
    residual_i = residual_i.copy()
    residual_i[~mask_i] = np.nan
    ...
    need_more, deltaC = process_stage2_residuals(
        residuals,
        stage2_positions,
        alpha=0.3,
        m_min=5,
...
- 某点长期没有足够有效观测时，达到 `m_max` 后返回 `NaN` 并告警；
- `need_more=True` 时不要生成最终修正表；应继续扫描。

如果 `residuals` 的扫描次数还没有达到 `m_min`，`deltaC` 会是 `None`。
...
`C_final` 会保持为完整有限表，作为最终插值的源表；还要同时保存 `interpolation_mask`：

```text
interpolation_mask = stage1_mask_on_stage2_grid OR stage2_valid
```

其中 `stage2_valid` 表示 `deltaC` 的 X/Y 两个分量都有限。

特别地：

```text
stage1_mask_on_stage2_grid == False 且 stage2_valid == False
```

的点虽然在 `C_final` 中保留了 fallback 值，但没有可靠校准依据，不得作为后续插值源点。
...
1. 非 Python 外部程序通过适配层调用本模块；通信协议可以自定，但必须保留数组形状、mask、数据类型和 NaN 信息。
...
5. 合并时 `initial_correction`、`deltaC` 和 `stage1_mask_on_stage2_grid` 必须属于同一个网格。
6. `C_final` 是合并后的源表；设备最终收到的是对 `C_final` 再插值后的 `final_download_table`。
7. 阶段 1、阶段 2 都失效的点可以保留 fallback 值供设备使用，但不能让它参与后续插值。